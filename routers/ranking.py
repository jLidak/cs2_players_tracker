...
RANKING_RATING_EXPONENT_DIV = (
    1.0  # Divisor for the rating differential exponent (damping)
)
    ...
    for player in players:
        ...
        for perf in player.tournament_performances:
            ...
            if tour.bracket_type == "Bracket 6 teams" and starts_in_semis:
                w_group = (
                    tour.weight_group_override
                    if tour.weight_group_override is not None
                    else tour.weight_group
                )
                tournament_points_sum += process_phase(
                    "Group (Override)", perf.rating_group, w_group, r_group
                )
            else:
                tournament_points_sum += process_phase(
                    "Group Stage", perf.rating_group, tour.weight_group, r_group
                )
            ...
            if tour.bracket_type == "Bracket 6 teams" and starts_in_semis:
                ...
                w_semi = (
                    tour.weight_semis_override
                    if tour.weight_semis_override is not None
                    else remaining / 2
                )
                w_final = (
                    tour.weight_final_override
                ...
                tournament_points_sum += process_phase(
                    "Final", perf.rating_final, w_final, r_final, bonus=RANKING_BONUS
                )
            else:
                tournament_points_sum += process_phase(
                    "Quarter-Final",
                    perf.rating_quarters,
                    tour.weight_quarters,
                    r_quarters,
                    bonus=RANKING_BONUS,
                )
                tournament_points_sum += process_phase(
    ...
    for player in players:
        ...
        for perf in player.tournament_performances:
            ...
            def process_custom_phase(
                ...
                exp_div = (
                    params.rating_exponent_divisor
                    if params.rating_exponent_divisor != 0
                    else 1.0
                )
                exponent = 1 / exp_div
                damped_diff = math.copysign(abs(diff) ** exponent, diff)
            ...
            if tour.bracket_type == "Bracket 6 teams" and starts_in_semis:
                points_sum += process_custom_phase(
                    "Group (Override)", perf.rating_group, w_group_ov, r_group, 0.0
                )
                points_sum += process_custom_phase(
                    "Semi-Final", perf.rating_semis, w_sf_ov, r_semis, params.bonus_sf
                )
                points_sum += process_custom_phase(
                    "Final", perf.rating_final, w_final_ov, r_final, params.bonus_final
                )
            else:
                points_sum += process_custom_phase(
                    "Group Stage", perf.rating_group, w_group, r_group, 0.0
                )
                points_sum += process_custom_phase(
                    "Quarter-Final",
                    perf.rating_quarters,
                ...
                points_sum += process_custom_phase(
                    "Semi-Final", perf.rating_semis, w_sf, r_semis, params.bonus_sf
                )
                points_sum += process_custom_phase(
                    "Final", perf.rating_final, w_final, r_final, params.bonus_final
                )
